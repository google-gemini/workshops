...
def screenshot_window_or_fallback(
    window_name, out_filename, retroarch_controller
):
  try:
...
def summarize_smash_detection(result):
  ...
  platform_phrase = (
      ...
      if same_platform
      else "They are on different platforms."
  )
...
def capture_fullscreen_screenshot_to_base64() -> str:
  ...
  with mss.mss() as sct:
    ...
    monitor = sct.monitors[
        1
    ]  # sct.monitors[0] is the virtual "all monitors" screen
...
def llm_planner(move_queue, cancel_event, retroarch):
  ...
  if not available_tools:
    logging.error(
        "LLM Planner: No tools defined or found. Planner will not run."
    )
    return
    ...
    try:
      ...
      if ai_msg.tool_calls:
        for tool_call in ai_msg.tool_calls:
          ...
          logging.info(
              f"LLM Planner: Gemini chose tool: {tool_name} with"
              f" args: {tool_args}"
          )
          ...
          if tool_name in tool_dispatch_table:
            ...
            logging.warning(
                f"LLM Planner: Tool '{tool_name}' chosen by LLM not"
                " found in dispatch table."
            )
      else:
        logging.info(
            "LLM Planner: Gemini did not suggest a tool call."
            f" Response: {ai_msg.content}"
        )
...
def make_retroarch():
  ...
  if not retro_controller.is_alive(retries=2, delay=0.5):  # Quick check
    logging.error(
        "RetroArch is not responding. Please ensure it's running and"
        " network commands are enabled."
    )
  ...
  smash_rom_path = os.path.expanduser(
      "~/var/roms/Super Smash Bros. (U) [!].z64"
  )
  ...
  logging.info(
      f"Attempting to load ROM: {smash_rom_path} with Core: {n64_core_path}"
  )
