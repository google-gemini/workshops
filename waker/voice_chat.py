...
class WindWakerVoiceChat:
        ...
        for fc in tool_call.function_calls:
            ...
            if fc.name == "see_game_screen":
                ...
                if screenshot_data:
                    ...
                    vision_analysis = await self.analyze_screenshot_with_gemini(screenshot_data, user_query)
                    result = {
                ...
                if song_name not in songs:
                    ...
                    result = {
                        "success": True,
                        "message": (f"Playing {song_name.replace('_', ' ').title()}!" f" {song_info['description']}."),
                        "status": "song_started",
                    }
                ...
                if hasattr(self, "sailing_task") and self.sailing_task and not self.sailing_task.done():
                ...
                self.sailing_task = asyncio.create_task(self.sailing_background_monitor(query))
    ...
    async def analyze_screenshot_with_gemini(self, screenshot_data, user_query=""):
        ...
        try:
            ...
            response = await asyncio.to_thread(
                ...
                contents=[
                    types.Part.from_bytes(data=image_bytes, mime_type=screenshot_data["mime_type"]),
                    prompt,
                ],
    ...
    def search_user_history(self, query="", max_recent=10):
        ...
        try:
            if query:
                ...
                memories = self.memory_client.search(query=query, user_id="wind_waker_player", limit=5)
                    ...
                    if analysis_text:
                        try:
                            ...
                            if parsed.get("noteworthy") is True:
                                observation = parsed.get("description", "Something noteworthy detected")
    ...
    async def analyze_sailing_scene(self, screenshot, user_query):
      ...
      """
            ...
            response = await asyncio.to_thread(
                ...
                model="gemini-2.0-flash-lite",
                contents=[
                    types.Part.from_bytes(data=image_bytes, mime_type=screenshot["mime_type"]),
                    prompt,
                ],
                config=types.GenerateContentConfig(response_mime_type="application/json"),
            )
