...
async def gather_crew_articles(crew_list) -> List[Dict]:
    ...
    for crew_member in crew_list:
        ...
        if crew_member.name not in unique_crew:
            unique_crew[crew_member.name] = {
                "name": crew_member.name,
                "jobs": [crew_member.job]
            }
        else:
...
async def gather_comprehensive_film_data(title: str, year: int, 
    ...
    print(f"   Cast articles: {len(cast_articles)} of {len(tmdb_data.credits.cast) if tmdb_data.credits and tmdb_data.credits.cast else 0}")
    print(f"   Crew articles: {len(crew_articles)} of {len(tmdb_data.credits.crew) if tmdb_data.credits and tmdb_data.credits.crew else 0}")
...
if __name__ == "__main__":
    ...
    async def test_data_gathering():
        ...
        try:
            document = await gather_comprehensive_film_data(
