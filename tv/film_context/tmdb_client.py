...
def search_movie_with_credits(title: str, year: int = None):
    ...
    if year:
        year_filtered = [movie for movie in search_results if movie.release_date and movie.release_date.year == year]
        if year_filtered:
    ...
    movie_details = tmdb.movie(first_result.id).details(append_to_response="credits")
